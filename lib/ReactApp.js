...
function WelcomeModalUsername(props) {
	...
	return React.createElement(
		...
		React.createElement("input", { type: "text", value: props.username, className: "form-control", onChange: handleUsernameChange })
	);
}
	...
	_createClass(UserDataComponent, [{
		...
		value: function getComments() {
			...
			axios.get('/getComments').then(function (res) {
				...
				if (_this8.state.comments.length > 2) {
					if (_this8.state.comments[0].id != data[0].id) {
						_this8.setState({ comments: data });
		...
		value: function render() {
			...
			return React.createElement(
				...
				React.createElement(CommentBox, { color: this.state.color, username: this.state.username, addCommentToState: this.addCommentToState }),
				modal
			);
		}
	}]);
